...
from data.get_datasets_tr_OLbasic_val_NN_aiims import get_datasets
...
def main(foolded):
    ...
    for epoch in range(cfg.epochs):
        ...
        if epoch % cfg.val_freq == 0:
            print('==> validation...')
            val_mae, best_k = validate(loader_dict, model, cfg, epoch)
...
def train(epoch, train_loader, model, optimizer, cfg, prev_loss_record):
    ...
    for idx, (x_base, x_ref, _, ranks, _) in enumerate(train_loader):
        ...
        embs_arr.append(embs[:16].detach().clone())
        ranks_arr.append(ranks[0].detach().clone())
        ...
        if idx % cfg.print_freq == 0:
            ...
            sys.stdout.flush()
    embs_all = torch.cat(embs_arr)
    ranks_all = torch.cat(ranks_arr)
    #writer.add_embedding(embs_all, metadata=ranks_all, global_step=epoch)
...
def get_index_from_files(files_array, cfg):
    obj = []
    for vals in files_array:
        obj.append([i.split("/")[-1][:-4] for i in vals])
    return np.array(obj)

accuracies = [0,0,0,0,0]

def validate(loader_dict, model, cfg, epoch):
    model.eval()
    data_time = AverageMeter()

    embs_train, fnames_train = extract_embs(model.encoder, loader_dict['train_for_val'], split="val")
    #import ipdb
    #ipdb.set_trace()
    #writer.add_embedding(embs_train, metadata=[int(i.split("_")[-1][:-4]) for i in fnames_train], global_step=epoch)
    
    embs_train = embs_train.cuda()


    embs_test, fnames = extract_embs(model.encoder, loader_dict['val'], split="val")
    ...
    files_ind_all = defaultdict(list)
    ...
    with torch.no_grad():
        ...
        for idx in range(n_batch):
            ...
            for k in cfg.k:
                ...
                files_array = np.array(fnames_train)[inds[:, :10]]
                files_ind = get_index_from_files(files_array, cfg) 
                pred_mean = np.round(np.mean(nn_labels, axis=-1, dtype=np.float32))
                preds_all[k].append(pred_mean)
                files_ind_all[k].append(files_ind)


    for key in preds_all.keys():
        preds_all[key] = np.concatenate(preds_all[key])
        files_ind_all[key] = np.concatenate(files_ind_all[key])
    for ind, i in enumerate([2,4,6,8,10]):
        acc = (abs(preds_all[i] - test_labels) <=1).sum()/len(test_labels)
        print("Accuracy for ", i, " is ", acc)
        for j in range(len(fnames)):
            print( int(preds_all[i][j]), " | ", test_labels[j], " | " ,fnames[j], " | " + ",".join(files_ind_all[i][j].astype(str)))
        if acc >= accuracies[ind] and i == 10:
            save_folder = save_address + "/" + str(ind)
            accuracies[ind] = acc
            #save model
            Path(save_folder).mkdir(parents=True, exist_ok=True)
            torch.save(model.state_dict(), save_folder + "/model.tar")
            print("Saving Model")
            #save reffrences saved automatically with model
            #save embeddings for train
            torch.save(embs_train, save_folder + '/train_embs.pt')
            with open(save_folder + '/fnames.pkl', 'wb') as f:
                pickle.dump(fnames_train, f)
...
if __name__ == "__main__":
    ...
    foolded = int(sys.argv[1])
    model_name = sys.argv[2]
    gpu_id = sys.argv[3]
    os.environ['CUDA_VISIBLE_DEVICES'] = gpu_id
    save_address = save_address + model_name + "/split_" + str(foolded)
    main(foolded)
