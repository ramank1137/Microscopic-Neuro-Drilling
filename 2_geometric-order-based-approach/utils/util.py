...
def get_box_params(file, src="/mnt/f/Work/Work/Dataset/boxes.json"):
...
import json
with open("/mnt/f/Work/Dataset/data_copy/boxes.json", "r") as rdfile:
    boxes = json.load(rdfile)


def get_homographic_transformation(file, side = 224):
    
    #box = get_box_params(file.replace(".png", ".json").split("/")[-1])
    box = boxes[file.split("/")[-1]]
    ...
    #transformed_image = cv2.resize(image, (side, side), interpolation = cv2.INTER_LINEAR)
    transformed_image = cv2.cvtColor(transformed_image, cv2.COLOR_BGRA2RGB)
    #import ipdb
    #ipdb.set_trace()
    image = Image.fromarray(transformed_image).convert('RGB')
    #angle = find_rotation_matrix(boxes[file.split("/")[-1]])
    #image = image.rotate(angle=angle, expand=True)
    #image = image.resize((side,side), resample=Image.Resampling.BILINEAR)
    return image

def load_one_image(img_path, width=224, height=224):
    img = get_homographic_transformation(img_path)
    return img
"""
...
def extract_embs(encoder, data_loader, split="train"):
    encoder.eval()
    embs = []
    inds = []
    fnames = []
    with torch.no_grad():
        if split == "val":
            for x_base, _, item, fname in data_loader:
                x_base = x_base.cuda()
